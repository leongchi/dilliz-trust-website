...
全站不得加入「定存」、「Fixed Deposit」、「投資」、「投资」或 `investment`。繁體及簡體的固定收益相關內容繼續使用「固定收益信託」；英文首次／正式／法律位置使用 `Fixed-Term Custodian Funding Plan`，重複 UI、會員卡及窄版位置使用 `Custodian Plan`。風險披露維持目前已核准意思，不應自行縮短、刪除或改成銀行存款式描述。
