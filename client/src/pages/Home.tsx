        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-metal-gold font-serif">
              {lang === "zh" ? "關於帝力斯信託" : "About DILLIZ Trust"}
            </h2>
            ...
            <div className="pt-6">
              <Link href="/about" className="btn-gold shadow-gold-glow py-3.5 px-8 font-bold text-xs tracking-widest uppercase inline-flex items-center gap-2">
                {lang === "zh" ? "了解更多品牌故事" : "Learn More Story"} <ArrowRight size={14} />
              </Link>
            </div>
          </div>
      ...
      <section className="py-24 bg-[#2b2b2b] border-t border-b border-white/5 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 space-y-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            ...
            {[
              { id: "asset", name: t("services.tab.asset", lang), icon: Shield, desc: lang === "zh" ? "安全合規的第三方資產託管與法律隔離保護" : "Secure third-party custody & legal isolation" },
              { id: "trust", name: t("services.tab.trust", lang), icon: Compass, desc: lang === "zh" ? "量身定制跨國資產信託，合規合法減輕稅務" : "Bespoke trust structures to minimize global taxes" },
              { id: "deposit", name: t("services.tab.deposit", lang), icon: Landmark, desc: lang === "zh" ? "協助在瑞士、新加坡等頂級離岸中心設立賬戶" : "Set up accounts in Swiss/Singapore financial centers" },
              { id: "finance", name: t("services.tab.finance", lang), icon: CreditCard, desc: lang === "zh" ? "定存資產直接授信，核發尊貴聯名黑金卡" : "Direct credit based on assets, black gold co-brand" },
              { id: "card", name: t("services.tab.card", lang), icon: Receipt, desc: lang === "zh" ? "一站式海外物業稅、子女學費等全球賬單代付" : "One-stop escrow & auto-payment for global bills" }
            ].map((srv, idx) => {
          ...
          <div className="text-center pt-4">
            <Link href="/services" className="btn-gold shadow-gold-glow py-3.5 px-8 font-bold text-xs tracking-widest uppercase inline-flex items-center gap-2">
              {lang === "zh" ? "探索核心服務詳情" : "Explore Full Services"} <ArrowRight size={14} />
            </Link>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-stretch">
          ...
          <div className="bg-white/5 border border-white/10 p-8 md:p-10 rounded-3xl flex flex-col justify-between hover:border-metal-gold/30 transition-all duration-300 relative overflow-hidden">
            ...
            <div className="space-y-6">
              ...
              <h3 className="text-2xl md:text-3xl font-bold text-slate-200 font-serif leading-tight">
                {lang === "zh" ? "360度互動案例情境" : "360° Interactive Case Studies"}
              </h3>
              <p className="text-slate-300 font-light text-sm leading-relaxed">
                {lang === "zh"
                  ? "我們將高淨值客戶面臨的創業債務風險、資產海外配置、突發意外凍結等六大情境，以「以客為尊 / YOU」為中心進行 360 度環狀推演，提供合規合法的定製信託解決方案。"
            ...
            <div className="pt-8 mt-8 border-t border-white/5">
              <Link href="/cases" className="btn-gold shadow-gold-glow py-3 px-6 font-bold text-xs tracking-widest uppercase inline-flex items-center gap-2 w-full justify-center">
                {lang === "zh" ? "進入 360° 互動案例" : "Enter Interactive Cases"} <ArrowRight size={14} />
              </Link>
            </div>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 p-8 md:p-10 rounded-3xl flex flex-col justify-between hover:border-metal-gold/30 transition-all duration-300 relative overflow-hidden">
            ...
            <div className="space-y-6">
              ...
              <h3 className="text-2xl md:text-3xl font-bold text-slate-200 font-serif leading-tight">
                {lang === "zh" ? "專屬定存與信託會員計劃" : "Exclusive Trust & Deposit Plans"}
              </h3>
              <p className="text-slate-300 font-light text-sm leading-relaxed">
                {lang === "zh"
              ...
              <div className="grid grid-cols-2 gap-4 text-xs font-medium pt-2">
                <div className="flex items-center gap-2 text-slate-300">
                  ...
                  <span>{lang === "zh" ? "定存美金10,000起" : "$10,000+ USD Fixed Deposit"}</span>
                </div>
                <div className="flex items-center gap-2 text-slate-300">
                  <Check size={14} className="text-metal-gold" />
                  <span>{lang === "zh" ? "豁免離岸賬戶行政費" : "Waived Offshore Admin Fees"}</span>
                </div>
                <div className="flex items-center gap-2 text-slate-300">
                  <Check size={14} className="text-metal-gold" />
                  <span>{lang === "zh" ? "黑金聯名信用卡" : "Black Gold Co-branded Card"}</span>
                </div>
                <div className="flex items-center gap-2 text-slate-300">
                  <Check size={14} className="text-metal-gold" />
                  <span>{lang === "zh" ? "24小時專屬客服熱線" : "24/7 Dedicated Hotline"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-8 mt-8 border-t border-white/5">
              <Link href="/membership" className="btn-gold shadow-gold-glow py-3 px-6 font-bold text-xs tracking-widest uppercase inline-flex items-center gap-2 w-full justify-center">
                {lang === "zh" ? "查看會員權益對照" : "View Membership Tiers"} <ArrowRight size={14} />
              </Link>
            </div>
          </div>
      ...
      <section className="py-20 bg-[#2b2b2b] border-t border-white/5 text-center relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto px-6 space-y-8 relative z-10">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white font-serif leading-tight">
            {lang === "zh" ? "立即開啟您的專屬財富傳承規劃" : "Begin Your Bespoke Wealth Planning Today"}
          </h2>
          ...
          <div className="flex flex-wrap justify-center gap-4 pt-4">
            ...
            >
              {lang === "zh" ? "預約專屬諮詢" : "Book Private Consultation"} <ArrowRight size={14} />
            </Link>
            <Link 
              href="/faq" 
